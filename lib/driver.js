...
var Driver = module.exports = function Driver() {
  ...
  this.events = [
    ...
    */
    ...
    /**
     * Emitted whenever a Mindflex packet is received over the serialport
     *
     * @event packet
     * @value packet
     */
...
Driver.prototype.parse = function(data) {
  ...
  while (reader.bytesAhead() >= 3) {
    if (reader.uint8() === MODE_1 && reader.uint8() === MODE_1) {
      this.emit("mode_1");
      continue;
    }
    ...
    if (reader.uint8() === BT_SYNC && reader.uint8() === BT_SYNC) {
      ...
      if (reader.bytesAhead() >= len + 1) {
        ...
        if (computedChecksum !== checksum) {
          this.emit("bad_packet", computedChecksum & checksum);
          continue;
        }
