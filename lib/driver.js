...
var Driver = module.exports = function Driver() {
  ...
  this.events = [
    ...
    */
    ...
    /**
     * Emitted whenever a Mindflex packet is parsed
     *
     * @event packet
     * @value parsed packet
     */
...
Driver.prototype.start = function(callback) {
  ...
  this.on("data", function(data) {
    var concatBuffer = Buffer.concat([this.buffer, data]);
    this.buffer = concatBuffer;
...
Driver.prototype.parse = function(data) {
  ...
  while (reader.bytesAhead() >= 3) {
    if (reader.uint8() === MODE_00 && reader.uint8() === MODE_00) {
      this.emit("mode00");
      continue;
    }
    ...
    if (reader.uint8() === BT_SYNC && reader.uint8() === BT_SYNC) {
      ...
      if (reader.bytesAhead() >= len + 1) {
        ...
        //compact the buffer after successfully reading a packet
        reader.compact();
      }
    }
  }
  ...
  while (reader.bytesAhead() > 0) {
    switch (reader.uint8()) {
      ...
      case CODE_SIGNAL_QUALITY:
        packet.signal = reader.uint8();
        break;

      case CODE_ATTENTION:
        packet.attention = reader.uint8();
        break;

      case CODE_MEDITATION:
        packet.meditation = reader.uint8();
        break;

      case CODE_BLINK:
        packet.blink = reader.uint8();
        break;

      case CODE_WAVE:
        reader.skip(1);
        packet.wave = reader.int16BE();
        break;

      case CODE_ASIC_EEG:
        packet.eeg = this.parseEEG(reader.buffer(24));
        break;
    }
  }
